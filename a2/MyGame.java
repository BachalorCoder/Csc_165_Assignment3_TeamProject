...
public class MyGame extends VariableFrameRateGame
{
	/*
	A2 requirement map
	Orbit camera controller: initializeGame, doOrbit, doElevate, doZoom, CameraOrbit3D in tage
	Second viewport overhead: initializeGame, setOverheadCamera, panOverhead, zoomOverhead
	HUD for both viewports: updateHUD, overHudLeftPx, overHudTopPx
	Ground plane and movement limits: buildObjects ground plane, doMove, doStrafe, updateJump
	Axes toggle: toggleAxesVisibility
	Node controllers: initializeGame, tryTakePhoto, RotationController and BobbingController
	Hierarchical photos: buildObjects hudIcons parented to dolphin, placePhotosOnWall reparent to root
	Global yaw: doYaw uses globalYaw in GameObject
	*/

	private static Engine engine;

	// Home sizing values are reused for building the walls and for photo placement on the back wall.
	...
	public void setPadMove(float v)   { padMove = v; }
	public void setPadStrafe(float v) { padStrafe = v; }
	...
	public void buildObjects()
	{
		...
		// A2 avatar: dolphin is treated as the player avatar on the ground plane.
		dol = new GameObject(GameObject.root(), dolS, dolTx);
		initialTranslation = (new Matrix4f()).translation(0f, 0f, 6f);
		initialScale = (new Matrix4f()).scaling(2.8f);
		...
		roofRight.setLocalRotation((new Matrix4f()).rotationZ(-roofAngle).rotateX((float)java.lang.Math.toRadians(90f)));
		roofRight.setLocalTranslation((new Matrix4f()).translation(halfW/2f, wallTopY + roofRise/2f, 0f));

		// A2 hierarchical objects: photos are children of the dolphin until they get placed on the home wall.
		for (int i = 0; i < 3; i++)
		{
			hudIcons[i] = new GameObject(dol, quadS, pyramidTx[i]);
	...
	public void initializeGame()
	{
		...
		orbitCam = new CameraOrbit3D(mainCam, dol);
		orbitCam.setRadius(4.5f);
		orbitCam.setElevationDeg(15f);
		orbitCam.setAzimuthDeg(180f);
		orbitCam.update();

		// A2 second viewport: overhead view of the same world with pan and zoom controls.
		...
		// A2 node controllers: photographed pyramid rotates, collected photo bobs while attached to dolphin.
		...
		im.associateActionWithAllKeyboards(Key.A, new YawAction(this, 1f),
				InputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		im.associateActionWithAllKeyboards(Key.D, new YawAction(this, -1f),
				InputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);

		im.associateActionWithAllKeyboards(Key.LEFT, new StrafeAction(this, -1f),
				InputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		im.associateActionWithAllKeyboards(Key.RIGHT, new StrafeAction(this, 1f),
				InputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
	...
	public void update()
	{
		...
		if (orbitCam != null) orbitCam.update();
	...
	private void updateHUD()
	{
		...
		if (!eventMsg.isEmpty()) top = top + "   |   " + eventMsg;
		...
		Vector3f p = dol.getWorldLocation();
		...
		if (x < pad) x = pad; // keep it on screen if the window is small
	...
	{
		return (new Vector3f(dol.getWorldForwardVector())).normalize();
	}

	private float distanceToHome()
	{
		return dol.getWorldLocation().distance(new Vector3f(0f, 0f, 0f));
	}
	...
	private void checkCrash()
	{
		...
		Vector3f dLoc = dol.getWorldLocation();
	...
	// A2 photo action: taking a photo reveals the icon, increments score, and activates pyramid movement.
	...
	// A2 overhead camera: UVN set to look straight down, then movement adjusts only the camera location.
	...
	public void zoomOverhead(float dy)
	{
		...
		if (overY < 10f) overY = 10f;
		if (overY > 120f) overY = 120f;

		setOverheadCamera(engine.getRenderSystem().getViewport("OVER").getCamera());
	}

	// A2 axes toggle: enableRendering and disableRendering keeps the scenegraph objects intact.
	...
	public void toggleHelp()
	{
		...
		if (helpPage == 1) showEvent("HELP KEYBOARD", 1.0);
		else if (helpPage == 2) showEvent("HELP GAMEPAD", 1.0);
		else showEvent("HELP OFF", 1.0);
	}

	// A2 orbit controls: orbit, elevation, and zoom update camera state without changing dolphin heading.
	...
	// A2 ground movement: x and z movement is allowed, y stays at 0 unless jump is active.
	...
	public void doStrafe(float dir, float time)
	{
		...
		Vector3f pos = dol.getWorldLocation();
		Vector3f rt = new Vector3f(dol.getWorldRightVector()).normalize();
	...
	private void updateJump(float dt)
	{
		...
		dol.setLocalLocation(new Vector3f(pos.x, newY, pos.z));
	}
	...
	private int winW() { return engine.getRenderSystem().getGLCanvas().getWidth(); }
	private int winH() { return engine.getRenderSystem().getGLCanvas().getHeight(); }

	private int overHudLeftPx() { return Math.round(OVER_LEFT * winW()); }

	// HUD top position is derived from viewport placement so the text stays inside the overhead viewport.
	private int overHudTopPx()  { return Math.round((1.0f - (OVER_BOTTOM + OVER_H)) * winH()); }
}